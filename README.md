...
We first fit a classification tree using all available predictors to examine which variables best distinguish individuals who used mental health services in the past five years from those who did not. Because decision trees are highly flexible and can adapt closely to the specific structure of a given dataset—especially when predictors are numerous and outcome rates are imbalanced—we applied K-fold cross-validation (K=10) to evaluate how trees of different sizes would be expected to perform on new data. Cross-validation helps prevent overfitting by repeatedly splitting the data into training and validation folds, providing a more reliable estimate of model performance than apparent (in-sample) error alone. We then applied the standard one-standard-error rule, which selects the simplest tree whose cross-validated error is not meaningfully worse than the minimum observed error.

<p align="center">
  <img src="global%20tree%20CV%20results.png" alt="Cross-validated relative error as a function of tree complexity" width="400">
</p>

<p align="center"><em>Figure 2. Cross-validated relative error as a function of tree complexity. The plot shows cross-validated relative classification error (with ±1 standard error bars) for trees of increasing complexity, estimated using 10-fold cross-validation. The dashed horizontal line denotes the one-standard-error threshold relative to the minimum cross-validated error. The selected model corresponds to the simplest tree whose error falls within this threshold, yielding a single-split tree.</em></p>
